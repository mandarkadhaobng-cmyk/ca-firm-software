-- ============================================================
-- Migration 012: Payroll Tables (UUID schema)
-- ============================================================
...
-- 2. Payroll runs (one per month per firm)
...
-- 5. Payroll status history
CREATE TABLE IF NOT EXISTS payroll_status_history (
  id             UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  payroll_run_id UUID NOT NULL REFERENCES payroll_runs(id) ON DELETE CASCADE,
...
-- ── Indexes ──────────────────────────────────────────────
CREATE INDEX IF NOT EXISTS idx_employee_salary_user   ON employee_salary(user_id);
CREATE INDEX IF NOT EXISTS idx_employee_salary_firm   ON employee_salary(firm_id, is_active);
CREATE INDEX IF NOT EXISTS idx_payroll_runs_firm      ON payroll_runs(firm_id, year DESC, month DESC);
CREATE INDEX IF NOT EXISTS idx_salary_slips_run       ON salary_slips(payroll_run_id);
CREATE INDEX IF NOT EXISTS idx_salary_slips_user      ON salary_slips(user_id, year DESC, month DESC);
CREATE INDEX IF NOT EXISTS idx_salary_slips_firm      ON salary_slips(firm_id);
CREATE INDEX IF NOT EXISTS idx_payroll_status_hist    ON payroll_status_history(payroll_run_id);
